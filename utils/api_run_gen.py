...
def unlimiform(model_name_or_path="meta-llama/Llama-2-7b-chat-hf", 
               model_type="llama",
               seed=42,
               n_gpu=1,
               no_cuda=False,
...
class generate:
    ...
    def __init__(self, 
        ...
        self.num_return_seqeuences=num_return_seqeuences
        self.k=k
        self.p=p
    ...
    # Different models need different input formatting and/or extra arguments
        # prefix = args.prefix if args.prefix else args.padding_text
        # print(prompt_text)
        prompt_text = f'{prompt_text}{self.suffix}'
        # prompt_text.to("cuda")
        ...
        # input_ids.to(device ="cuda:0")
        # 
       


        max_seq_length = getattr(self.model.config, "max_position_embeddings", 0)
        length = adjust_length_to_model(self.length, max_sequence_length=max_seq_length)
        ...
        output_sequences = self.model.generate(
            ...
            do_sample=True,
            num_return_sequences=self.num_return_seqeuences,
            streamer= None
        )
        ...
        for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
            ...
            # Decode text
            # text = tokenizer.decode(generated_sequence, clean_up_tokenization_spaces=True)
            prompt_length = min(input_ids.shape[-1], self.model.unlimiformer.window_size())
            completion = self.tokenizer.decode(generated_sequence[prompt_length:])
            ...
            generated_sequences.append(completion.replace("</s>", ""))
            # print(total_sequence)
